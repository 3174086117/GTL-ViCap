...
class MSVD_Feats_DataLoader(Dataset):
    ...
    def __init__(
        ...
        self.noun_prior_feats = pickle.load(open(r"C:\Users\admin\Desktop\YangYang\V2T-CLIP4Caption-Reproduction-main\MSVD_Ns_Detect\MSVD_video_nouns_aligned_features508.pkl", "rb"))
        self.verb_prior_feats = pickle.load(open(r"C:\Users\admin\Desktop\YangYang\V2T-CLIP4Caption-Reproduction-main\MSVD_Vs_Detect\MSVD_video_verbs_aligned_features508.pkl", "rb"))
    ...
    def _get_video(self, choice_video_ids):
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            video_slice = np.concatenate([video_noun_feat, video_verb_feat, video_slice], axis=0)
            # video_slice = np.concatenate([video_noun_feat, video_slice], axis=0)
            ...
            slice_shape = video_slice.shape
            real_length = slice_shape[0]  # 真实帧长度

            max_video_length[i] = max_video_length[i] if max_video_length[i] > real_length else real_length

            if len(video_slice) < 1:
                print("video_id: {}".format(video_id))
            else:
                video[i][:real_length] = video_slice
                # ===================== 关键修复 =====================
                video_mask[i, :real_length] = 1  # 有效帧 = 1
